...
class JatsDom {
	 ...
	 */
	function JatsDom(&$request, &$plugin, &$journal, &$article) {
		// Configure the DOM.
	...
	function &rootElement() {
		...
		XMLCustomWriter::setAttribute($rootElement, 'article-type', $this->getNamespace());
		XMLCustomWriter::setAttribute($rootElement, 'xmlns:xsi', JATS_XSI);
		if ($this->getXMLSchemaVersion() != '') {
			XMLCustomWriter::setAttribute($rootElement, 'version', $this->getXMLSchemaVersion());
		}
		XMLCustomWriter::setAttribute($rootElement, 'xmlns:mml', MATHML_MML);
		XMLCustomWriter::setAttribute($rootElement, 'xmlns:xlink', XLINK_XMLNS);
		XMLCustomWriter::setAttribute($rootElement, 'dtd-version', JATS_SCHEMAVERSION);
