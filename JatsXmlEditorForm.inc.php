...
class JatsXmlEditorForm extends Form {
	/** @var $request object */
	var $request;
	 ...
	 * @param $plugin object
	 */
	function JatsXmlEditorForm(&$plugin, $request) {
		...
		$this->addCheck(new FormValidatorCustom($this, 'tempFileId', 'required', 'plugins.generic.jatsXmlEditor.submissionRequired', create_function('$tempFileId', 'return $tempFileId > 0;')));
		...
		$this->addCheck(new FormValidatorCustom($this, 'destination', 'required', 'plugins.generic.jatsXmlEditor.issueRequired', create_function('$destination, $form', 'return $destination == \'queue\'? true : ($form->getData(\'issueId\') > 0);'), array(&$this)));
